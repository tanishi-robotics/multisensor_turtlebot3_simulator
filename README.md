...
The Docker runtime is GPU-based and requires NVIDIA Container Toolkit on the host.
...
For lower-load execution without the Gazebo GUI client, use headless mode.
When started through `bringup.sh`, RViz is not auto-started in headless mode.
...
Add `--headless` to start only `gzserver` without `gzclient`.
...
Docker runtime は GPU 前提で、host 側に NVIDIA Container Toolkit が必要です。
...
Gazebo GUI client を起動せずに負荷を下げたい場合は、headless mode を使います。
`bringup.sh` 経由の headless mode では RViz も自動起動しません。
...
`--headless` を追加すると、`gzclient` を起動せず `gzserver` のみを起動します。
